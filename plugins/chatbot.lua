...
function plugin.onTextMessage(msg, blocks)
	...
	local chatterdat = u.load_data('dados/chatbot/chatter.json')

	if msg.text == 'Fica calado moderador' then
      ...
      return
		...
		u.save_data('dados/chatbot/chatter.json', chatterdat)
		api.sendMessage(msg.chat.id, 'Vou ficar na minha', true)

		return
	end

	if msg.text == 'Pode falar moderador' then
		if not u.is_mod(msg) then
			api.sendMessage(msg.chat.id, 'Você não é admin. Me deixa em paz.', true)
      return
    end
		...
		u.save_data('dados/chatbot/chatter.json', chatterdat)
		api.sendMessage(msg.chat.id, 'Vamos teclar', true)

		return
	end

	chatterdat = u.load_data('dados/chatbot/chatter.json')

    if chatterdat[tostring(msg.chat.id)] == nil then
		chatterdat[tostring(msg.chat.id)] = true

		u.save_data('dados/chatbot/chatter.json', chatterdat)
		api.sendMessage(msg.chat.id, 'Olá, estou de volta', true)
	...
	palavrachave, j = string.find(msg.text, 'moderador')
	...
	var03, j = string.find(msg.text, '^ed[@'..bot.username..']*')  
	if var03 ~= nil then 
		resposta = 'Quem ė esse ed?' 
	end
	var04, j = string.find(msg.text, 'quem e v[@'..bot.username..']*') 
	if var04 ~= nil then  
		resposta = 'Moderador de grupos... com o objetivo de administrar grupos.' 
	end
	var05, j = string.find(msg.text, 'conpet[@'..bot.username..']*') 
	if var05 ~= nil then  
		resposta = 'competição ?\nAmo competir em quem escreve mais rapido.' 
	end
	var06, j = string.find(resposta, 'foto') 
	if var06 ~= nil then  
		resposta = 'Eu não entendo muito sobre fotos, mais geralmente estou sempre postando fotos em meu novo canal http://adfoc.us/43421365443984' 
	end
	var07, j = string.find(msg.text, 'quem te cri[@'..bot.username..']*') 
	if var07 ~= nil then  
		resposta = 'Desenvolvido by:\n@viniciusvrc\nApoio server @Mkriel' 
	end

	resposta = string.gsub(resposta, 'moderador ', 'Ed ') -- # Troca o nome 'Ed ' por 'WoW'. Altere 'WoW' para o que desejar
	resposta = string.gsub(resposta, 'moderador.', 'Ed.') -- # Troca o nome 'Ed ' por 'WoW'. Altere 'WoW' para o que desejar
